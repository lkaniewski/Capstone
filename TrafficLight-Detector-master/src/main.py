...
# Time: 19:20
#
#need to crop image to the box size so then only one color being grabbed 
import os
import cv2
import numpy as np
...
def objectDetect(inputPath, file):
    print(inputPath+file)
    model_path = "./models/yolo-tiny.h5"
    input_path = inputPath+file #"./input/housing_1.jpg"
    ...
    return max_score and num_trafficLight
...
if __name__ == '__main__':
    ...
    for f in os.listdir(path):
        print (f)
        objectDetect(path, f)
        if f.endswith('.jpg') or f.endswith('.JPG') or f.endswith('.png') or f.endswith('.PNG') and isTrafficLight(num_trafficLight, max_score):
            detect(path, f)
            print('--')
